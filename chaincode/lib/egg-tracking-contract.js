...
class EggTrackingContract extends Contract {
  ...
  async createMedicine(ctx, doctorId, medicineId, medicineName, quantity) {
    let identity = ctx.clientIdentity;
    ...
    // update state with new egg box
    await ctx.stub.putState(key, medicine.serialise());
  ...
  async createShipment(ctx, farmerId, shipperId, distributorId, shipmentCreation, min, max) {
    ...
    // if (!this.isDoctor(identity) && !this.isAdmin(identity)) {
    //   throw new Error(`Shipment can only be created by doctors or administrators`);
    // }
